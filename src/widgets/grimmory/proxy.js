...
const proxyName = "grimmoryProxyHandler";
const sessionTokenCacheKey = `${proxyName}__sessionToken`;
const logger = createLogger(proxyName);

async function login(widget, service) {
  if (!widget.username || !widget.password) {
    logger.debug("Missing credentials for Grimmory service '%s'", service);
  ...
  if (status !== 200) {
    logger.debug("Grimmory login failed for service '%s' with status %d", service, status);
    return { accessToken: false };
  }
  ...
  try {
    ...
    logger.error("Unable to login to Grimmory API: %s", e);
  }
...
async function apiCall(widget, endpoint, service) {
  ...
  if (status === 401 || status === 403) {
    logger.debug("Grimmory API rejected the request, attempting to obtain new session token");
  ...
  if (status !== 200) {
    logger.error("Error getting data from Grimmory: %s status %d. Data: %s", url, status, data);
    return { status, data: null };
  }
  ...
  try {
    ...
    logger.error("Error parsing Grimmory response: %s", e);
  }
...
export default async function grimmoryProxyHandler(req, res) {
  ...
  if (!widget.username || !widget.password) {
    logger.debug("Missing credentials for Grimmory widget in service '%s'", service);
    return res.status(400).json({ error: "Missing Grimmory credentials" });
  }
